...
export default function DistanceCalculator() {
  ...
  const [isMounted, setIsMounted] = useState(false) // New state for client-side check
  ...
  // Hook to set isMounted to true after the component has hydrated on the client
  useEffect(() => {
    setIsMounted(true)
  }, [])
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {distanceData && (
        <>
          <TravelTimeDisplay distanceKm={distanceData.distanceKm} planetName={distanceData.planet} />
          <PlanetInfoCard planetId={selectedPlanet} />
        </>
      )}

      {/* 3D Visualization */}
      {isMounted && ( // Conditional render for guaranteed client-side mount
